...
class UserSearchRepository {
  ...
  Future<void> _debugListAllUsers() async {
    try {
      ...
      print('📋 Found ${allUsers.docs.length} users:');
  ...
  Future<void> _debugCheckUserByEmail(String email) async {
    try {
      print('🔍 DEBUG: Checking for user with email: $email');
      ...
      if (query.docs.isEmpty) {
        ...
        if (matching.isNotEmpty) {
          ...
          print('❌ No users found with any case variation of email: $email');
        }
      } else {
        print('✅ Found user:');
        print('----------------------------------------');
  ...
  Future<List<AppUser>> searchUsers(String query) async {
    ...
    try {
      ...
      // Execute queries and debug checks in parallel
      print('🔍 Executing queries and debug checks...');
      final results = await Future.wait([
        emailQuery.get(),
        nameQuery.get(),
        _debugListAllUsers(),
        if (searchTerm.contains('@')) _debugCheckUserByEmail(searchTerm),
      ]);
      
      // Extract the query results (first two items in the array)
      final emailResults = results[0] as QuerySnapshot;
      final nameResults = results[1] as QuerySnapshot;
      ...
      final uniqueDocs = allDocs.fold<Map<String, dynamic>>(
        ...
        (map, doc) {
          ...
          print('� Data: ${doc.data()}');
          return map..putIfAbsent(doc.id, () => doc.data() as Map<String, dynamic>..['uid'] = doc.id);
        },
      );
      
      print('🔍 Found ${uniqueDocs.length} unique results');
